...
(defun describe-handles ()
  (sb-thread:with-mutex (sbcl-librarian::*handle-lock*)
    (format t "SBCL-LIBRARIAN HANDLES~%")
    (format t "     HANDLE | TYPE~%")
    (loop :for handle :being :the :hash-keys :of (aref sbcl-librarian::*handles* 0)
...
(defun handle-count ()
  (sb-thread:with-mutex (sbcl-librarian::*handle-lock*)
    (hash-table-count (aref sbcl-librarian::*handles* 0))))
